...
## 4. Instantiating a `NibLoadable` UIViewController

`Reusable` also allows you to mark your `UIViewController` classes as `NibLoadable` to easily instantiate them from their associated XIB in a type-safe way.

## 4a. Declare your `UIViewController` to conform to `NibLoadable` 

In your swift source declaring your custom `UIViewController` class:

* Use the `NibLoadable` protocol if you used a "File's Owner" of the XIB being of the class of your custom view controller.

```swift
// a XIB-based custom UIViewController, used as the XIB's "File's Owner"
final class CustomViewController: UIViewController, NibLoadable { /* and that's it! */ }

```

## 4b. Instantiate your UIViewControllers
